...
if(distance<=30)
{    console.log("distance<=30"+distance);
...
$(".gbutton").prop('disabled', false);
  ...
  if(distance==10){
    console.log("distance<=10"+distance);
    var futureid=$('#futureid').val();
generateGameresult(futureid);
    generateGameid();
  }
}
...
//generateGameresult(futureid);
    ...
    success: function (html) {
     //alert(html);
	 var arr = html.split('~');
	 console.log("generateGameid="+arr);
	 document.getElementById("gameid").innerHTML=arr[1];
	 document.getElementById("inputgameid").value=arr[1];
	 document.getElementById("futureid").value=arr[2];
      return false;
      },
      error: function (e) {}
      });
	}
  function generateGameresult(gamid)
{

	$.ajax({
    type: "Post",
    data:"futureid=" + gamid + "& type=" + "generate" ,
    url: "winnerResult.php",
    success: function (html) {
     //alert(html);
	 
      console.log("resultgenrated"+html);
      },
      error: function (e) {}
      });
	}
