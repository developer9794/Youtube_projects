...
{?>
    ...
    <p> <i class="icon ion-md-trophy"></i> <div class="title">Parity Record</div> </p>
        ...
        <tbody>
        <?php
 //$parityrecordQuery=mysqli_query($con,"select * from `tbl_result` where `tabtype`='parity' order by id desc limit 20");
 $parityrecordQuery=mysqli_query($con,"SELECT periodid, MAX(id) AS id, MAX(price) AS price, MAX(randomprice) AS randomprice, MAX(result) AS result, MAX(randomresult) AS randomresult, MAX(color) AS color, MAX(randomcolor) AS randomcolor, MAX(resulttype) AS resulttype, MAX(tabtype) AS tabtype, MAX(createdate) AS createdate
 FROM tbl_result
 GROUP BY periodid order by id desc limit 20");
