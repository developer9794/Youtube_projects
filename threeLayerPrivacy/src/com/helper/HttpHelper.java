...
public class HttpHelper {
	...
	public static HashMap parseMultipartRequest(HttpServletRequest request) {
		...
		if (!isMultipart) {
			...
			while (itr.hasNext()) {
				...
				System.out.println("File Item is:"+item);
				if (item.isFormField()) {
					...
					try {
						param.put(item.getFieldName() + "FILE",item.getInputStream());
	...
	public static HashMap displayRequest(HttpServletRequest request) {
		...
		while (requestAttributes.hasMoreElements()) {
			try {
				String attributeName = StringHelper.n2s(requestAttributes
						.nextElement());
				String attributeValue = StringHelper.n2s(request
